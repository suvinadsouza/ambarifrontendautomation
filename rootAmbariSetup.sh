#!/bin/bash
#full ambari setup script; the script combines cleanup, prereq setup, ldap setup, ssl setup and root and root setup based on user preferences
...
echo -e "\n Path to your hostdetails file is $path_to_hostdetails \n"
...
#echo -e "Enter the ambari server head node FQDN\n"
#read ambariServer
...
#reset root passwordless ssh if it was removed for root setup:#test4
...
: '
echo -e "\nDo you want to clean up existing ambari-server and value adds for root user(yes/no)?\n"
read cleanup
'
...
for i in "${hostnamesArray[@]}"
do
#clean up existing install of bigsql
sshpass -p $rootpasswd ssh -o StrictHostKeyChecking=no root@$i /tmp/installautomation/bigsqlcleanup_root_user.sh $pathToRepoFiles $pathToExtScripts &
echo -e "\n waiting... \n"
wait
#cleanup existing install of ambari server
sshpass -p $rootpasswd ssh -o StrictHostKeyChecking=no root@$i /tmp/installautomation/ambari_root_cleanup.sh $pathToRepoFiles &
wait
echo -e "\n waiting... \n"
done
fi
...
: '
echo -e "Do you want to configure prerequisites for ambari/hdp setup(yes/no)?\n"
read prerequisites
'
#set pre-requisites : test7
if [ "$prerequisites" == "yes" ]
then
echo -e "started prereq setup \n"
sleep 1s
for i in "${hostnamesArray[@]}"
do
echo -e "you are in host $i \n"
...
: '
echo -e "Do you want to configure ldap for root user(yes/no)?\n"
...
for i in "${hostnamesArray[@]}"
do
sshpass -p $rootpasswd ssh $i -t -l root "telnet $ldapServer"
sleep 2s
echo -e "\nyou are executing for node $i\n"
/tmp/installautomation/replicateIOPLDAP-bdvm004.sh $i 2> /dev/null &
wait
echo -e "\ncompleted ldap users creation on node $i\n"
...
#passwordless ssh must be setup from ambari head to all nodes including itself and from bigsql head to all workers: test9
sshpass -p $rootpasswd ssh $ambariServer  -t -l root "/tmp/installautomation/setupSSH.sh $rootpasswd root"
echo -e "\n waiting 2secs \n"
sleep 2s
...
sshpass -p $rootpasswd ssh $bigsqlHead  -t -l root "/tmp/installautomation/setupSSH.sh $rootpasswd root"
echo -e "\n waiting 2secs \n"
sleep 2s 
fi
...
: '
echo -e "\nDo you want to install ambari-agent for root user(yes/no)?\n"
read ambari-agent
'
#set up ambari agent: test10
if [ "$ambari_agent" == "yes" ]
then
#ambari agents need to be set up on all nodes in the cluster
for i in "${hostnamesArray[@]}"
do
echo -e "\nyou are installing ambari agent on node $i\n"
sshpass -p $rootpasswd ssh -o StrictHostKeyChecking=no root@$i /tmp/installautomation/root_ambari_agent_setup.sh $path_to_hostdetails
echo -e "\n waiting 5secs \n"
sleep 5s
#sshpass -p $rootpasswd ssh $i -t -l root " chmod -R ugo+rw /var/lib/ambari-agent/cache/stacks/BigInsights/*/services/"
sshpass -p $rootpasswd ssh $i -t -l root "chmod -R ugo+rw /var/lib/ambari-agent/cache/stacks/HDP/*/services"
done
fi

: '
echo -e "\nDo you want to install ambari-server for root user(yes/no)?\n"
read ambari-server
'
#setup ambari server: test11
if [ "$ambari_server" == "yes" ]
then
sshpass -p $rootpasswd ssh -o StrictHostKeyChecking=no root@$ambariServer  yum install ambari-server -y
sleep 2s
sshpass -p $rootpasswd ssh -o StrictHostKeyChecking=no root@$ambariServer  updatedb &
wait
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /etc/rc.d/init.d/ambari-server setup"
echo -e "\n waiting 15secs \n"
sleep 3s
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /etc/rc.d/init.d/ambari-server start"
#sshpass -p $rootpasswd ssh $ambariServer  -t -l root " chmod -R ugo+rw /var/lib/ambari-server/resources/stacks/BigInsights/*/services/"
sshpass -p $rootpasswd ssh $ambariServer  -t -l root "sudo chmod -R ugo+rw /var/lib/ambari-server/resources/stacks/HDP/*/services"
fi
...
'
#setup two way ssl between agents and server: test12
if [ "$twoWaySSLResponse" == "yes" ]
then
sshpass -p $rootpasswd ssh $ambariServer  -t -l root "chmod 777 /etc/ambari-server/conf/ambari.properties"
sshpass -p $rootpasswd ssh $ambariServer  -t -l root 'sudo echo "security.server.two_way_ssl=true" >> /etc/ambari-server/conf/ambari.properties'
sleep 1s
...
then
echo -e "\n waiting 70secs \n"
sshpass -p $rootpasswd ssh $ambariServer -t -l root "telnet $ldapServer"
sleep 2s
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /usr/sbin/authconfig --enableldap --enableldapauth --ldapserver=ldap://$ldapServer:$ldapserver_port --ldapbasedn='dc=ibm,dc=com' --update"
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /etc/rc.d/init.d/ambari-server setup-ldap"
sleep 2s
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /etc/rc.d/init.d/ambari-server restart"
sleep 2s
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /etc/rc.d/init.d/ambari-server sync-ldap --all"
sleep 2s
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /etc/rc.d/init.d/ambari-server restart"
sleep 2s
sshpass -p $rootpasswd ssh $ambariServer  -t -l root " /etc/rc.d/init.d/ambari-server sync-ldap --all"
...
: '
echo -e "\nDo you want to configure ssl for root user(yes/no)?\n"
read SSLuserResponse
'
#setup ssl: test14
if [ "$SSLuserResponse" == "yes" ]
then
sshpass -p $rootpasswd ssh $ambariServer  -t -l root "/tmp/installautomation/rootsslsetup.sh $ambariServer"
echo -e "\n waiting 3secs \n"
sleep 3s
else
sshpass -p $rootpasswd ssh $ambariServer  -t -l root "chmod 777 /etc/ambari-server/conf/ambari.properties"
sshpass -p $rootpasswd ssh $ambariServer  -t -l root 'echo "client.api.port=8081" >> /etc/ambari-server/conf/ambari.properties'
sshpass -p $rootpasswd ssh $ambariServer  -t -l root "/etc/rc.d/init.d/ambari-server restart"
fi

: '
echo -e "\nDo you want to configure local kdc server for root user(yes/no)?\n"
...
sshpass -p $rootpasswd ssh $KDCServer  -t -l root "/tmp/installautomation/rootlocalKDCSetup.sh $path_to_hostdetails"
echo -e "\n waiting 5secs \n"
sleep 5s
fi
