...
sudo wget -P /etc/yum.repos.d $ambari_repo_path
sudo yum repolist all
sudo sleep 2s 
sudo yum -y install ambari-agent
#echo -e "enter the FQDN of ambari node\n"
#read ambarinode
#echo -e "enter the non root username\n"
#read nonrootuser

sudo chmod ugo+rwx /etc/ambari-agent/conf/ambari-agent.ini
sudo sed -i -e 's/'"hostname=localhost"'/'"hostname=$ambarinode"'/g' /etc/ambari-agent/conf/ambari-agent.ini
sudo sed -i -e 's/'"run_as_user=root"'/'"run_as_user=$nonrootuser"'/g' /etc/ambari-agent/conf/ambari-agent.ini
sudo updatedb
